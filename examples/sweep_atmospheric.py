"""
Severity sweep for blur_atmospheric_turbulence_v1.

Produces `sweep_atmospheric.png`: a grid showing the input image and
its response to seven increasing severities, all rendered from the
same seed so the differences are attributable purely to severity.

Only requires PIL + numpy (no torchvision).

Run from anywhere:
    python examples/sweep_atmospheric.py --out sweep_atmospheric.png
    python -m examples.sweep_atmospheric  --out sweep_atmospheric.png

The script is self-contained: it prepends the repo root to ``sys.path``
so ``python examples/sweep_atmospheric.py`` works from the repo root
...
from distortion_library.blur_atmospheric_turbulence_v1 import (
    blur_atmospheric_turbulence_v1 as atm,
)
...
def _load_image(path: Optional[str], device: torch.device) -> torch.Tensor:
    if path is None:
        ...
        # coloured rectangles — the checkerboard exposes aliasing from
        # the shimmer, the rectangles expose structure retention, the
        # gradient exposes global shift.
        H = W = 256
...
def main() -> None:
    ...
    ap.add_argument("--out", type=str, default="sweep_atmospheric.png")
    ...
    for s in SEVERITIES:
        y, _ = atm(x, severity=s, seed=args.seed)
        tiles.append(y.detach().cpu())

    out_path = Path(args.out)
    _save_grid(tiles, out_path, nrow=4, pad=4, pad_value=1.0)
    print(
